...
export function parsePoint(ewkb: Buffer): string {
  ...
  if ((type & 0xFFFF) !== 1) throw new Error('Not a Point geometry');
...
export function parseLineString(ewkb: Buffer): string {
  const view = new DataView(ewkb.buffer, ewkb.byteOffset);
  const isLittleEndian = view.getUint8(0) === 1;
  const type = view.getUint32(1, isLittleEndian);
  const srid = view.getUint32(5, isLittleEndian);
  
  if ((type & 0xFFFF) !== 2) throw new Error('Not a LineString geometry');
